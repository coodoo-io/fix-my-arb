...
function fixErrors(text: any): any {
  ...
  for (let i = 1; i < lines.length - 1; i++) {
    ...
    // warn used number as first char
    warnUsedNumberAsFirstChar(lines[i], i);
  }
...
function warnUsedNumberAsFirstChar(line: any, index: any): any {
  ...
  if (!isNaN(firstChar)) {
    vscode.window.showInformationMessage(
      "Your arb file has a number as the first character on line " +
        (index + 1) +
...
function fixColonSpace(line: any, index: any): any {
  ...
  if (parts.length < 2) {
    vscode.window.showInformationMessage(
      "Your arb file has a missing : on line " +
        (index + 1) +
        "\nPlease fix it and try again."
    );
    return line;
  ...
  if (parts.length > 2) {
    vscode.window.showInformationMessage(
      "Your arb file has an extra : on line " +
        (index + 1) +
        "\nPlease fix it and try again."
    );
    return line;
